...
public OnPluginStart()
{
    ...
    RegConsoleCmd("sm_sal", TankSal_Cmd, "Shuffles the tank again on player request.");
...
{ 
	if (L4D2Team:GetClientTeam(client) == L4D2Team:L4D2Team_Infected)
	{
		...
		{
			CPrintToChat(client, "{blue}[{default}Boss{blue}] {default}Sadece tank olacak kişiler salabilir.");
		}
		else
		{
			chooseTank();
...
public Action:Tank_Cmd(client, args)
{
    ...
    if (tankClientId != -1)
    {
        ...
        {
            if (client == tankClientId) CPrintToChat(client, "{blue}[{default}Boss{blue}] {default}Sonraki Tank: {green}SEN {default}(Salmak istersen !sal yaz.)");
            else CPrintToChat(client, "{blue}[{default}Boss{blue}] {default}Sonraki Tank: {olive}%s", tankClientName);
        }
    }
...
{    
    ...
    {
        PrintHintText(tank_index, "!!! 2. KONTROLE GECTIN !!!");
        for (new i = 1; i <= MaxClients; i++) 
        {
            if (! IsClientInGame(i) || ! IsInfected(i))
                continue;

            if (tank_index == i) CPrintToChat(i, "{blue}[{default}Tank Kontrol{blue}] {default}2. Kontrole geçtin.");
            else CPrintToChat(i, "{blue}[{default}Tank Kontrol{blue}] {default}Tank ({green}%N{default}) 2. kontrole geçti.", tank_index);
        }
...
public outputTankToAll()
{
    ...
    if (tankClientId != -1)
    {
        ...
        {
            CPrintToChatAll("{blue}[{default}Boss{blue}] {default}Sonraki Tank: {olive}%s", tankClientName);
        }
        else
        {
            for (new i = 1; i <= MaxClients; i++) 
            {
                ...
                if (tankClientId == i) CPrintToChat(i, "{blue}[{default}Boss{blue}] {default}Sonraki Tank: {green}SEN {default}(Salmak istersen !sal yaz.)");
                else CPrintToChat(i, "{blue}[{default}Boss{blue}] {default}Sonraki Tank: {olive}%s", tankClientName);
            }
        }
    }
}
