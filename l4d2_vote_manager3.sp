...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_slam", SlamVote, "SLAM engelleme için oylama");
...
public Action:SlamVote(client, args)
{
	ServerCommand("sm_customvote 5 \"SLAM engellensin mi?\" \"exec no_slam\"");
}
...
stock VoteManagerPrepareVoters(team)
{
	for(new i=1;i<=MaxClients;i++)
	{
		if(IsClientInGame(i) && !IsFakeClient(i))
		{
			...
			else if(team == 5 && GetClientTeam(i) != 1)
			{
				iVote[i] = Voted_CanVote;
			}
		}
...
stock StringToLower(String:string[], stringlength)
{
	...
	decl String:buffer[maxlength], String:puto[maxlength];
	Format(buffer, maxlength, string);
	
	for(new i;i<=stringlength;i++)
	{
		Format(puto, maxlength, buffer[i]);
		if(strlen(buffer[i+1]) > 0) ReplaceString(puto, maxlength, buffer[i+1], "");
		if(IsCharUpper(puto[0]))
		{
			puto[0] += 0x20;
			//CharToLower(char[0]); this fails for some reason
			Format(puto, maxlength, "%s%s", puto, buffer[i+1]);
			ReplaceString(buffer, maxlength, puto, puto, false);
		}	
	}
