...
MatchModeMenu(client)
{
	...
	SetMenuTitle(hMenu, "Kategori secin:");
...
public MatchModeMenuHandler(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		if (KvJumpToKey(g_hModesKV, sInfo) && KvGotoFirstSubKey(g_hModesKV))
		{
			...
			Format(sBuffer, sizeof(sBuffer), "%s icin config secin:", sInfo);
		...
		{
			CPrintToChat(param1, "{blue}[{default}match_vote{blue}] {default}Belirttiğinizle uyuşan config bulunamadı.");
...
bool:StartMatchVote(client, const String:cfgname[])
{
	...
	{
		CPrintToChat(client, "{blue}[{default}match_vote{blue}] {default}Izleyiciler bu komudu kullanamaz.");
		return false;
	}
	if (LGO_IsMatchModeLoaded())
	{
		CPrintToChat(client, "{blue}[{default}match_vote{blue}] {default}Ilk önce mevcut config kapatılmalıdır. (Komut: !rmatch)");
		return false;
	}
	if (!IsBuiltinVoteInProgress())
	{
		...
		{
			CPrintToChat(client, "{blue}[{default}match_vote{blue}] {default}Match vote cannot be started. Not enough players.");
			return false;
		}
		new String:sBuffer[64];
		g_hVote = CreateBuiltinVote(VoteActionHandler, BuiltinVoteType_Custom_YesNo, BuiltinVoteAction_Cancel | BuiltinVoteAction_VoteEnd | BuiltinVoteAction_End);
		Format(sBuffer, sizeof(sBuffer), "%s config yuklensin mi?", cfgname);
	...
	CPrintToChat(client, "{blue}[{default}match_vote{blue}] {default}Lütfen biraz bekledikten sonra tekrar deneyiniz.");
	return false;
}
...
public MatchVoteResultHandler(Handle:vote, num_votes, num_clients, const client_info[][2], num_items, const item_info[][2])
{
	for (new i=0; i<num_items; i++)
	{
		if (item_info[i][BUILTINVOTEINFO_ITEM_INDEX] == BUILTINVOTES_VOTE_YES)
		{
			...
			{
				DisplayBuiltinVotePass(vote, "Config yukleniyor...");
...
StartResetMatchVote(client)
{
	...
	if (!LGO_IsMatchModeLoaded())
	{
		CPrintToChat(client, "{blue}[{default}match_vote{blue}] {default}Yüklü bir config bulunamadı.");
		return;
	}
	if (IsNewBuiltinVoteAllowed())
	{
		...
		{
			CPrintToChat(client, "{blue}[{default}match_vote{blue}] {default}Henüz sunucuya bağlanmakta olan oyuncular varken kullanılamaz.");
			return;
		}
		g_hVote = CreateBuiltinVote(VoteActionHandler, BuiltinVoteType_Custom_YesNo, BuiltinVoteAction_Cancel | BuiltinVoteAction_VoteEnd | BuiltinVoteAction_End);
		SetBuiltinVoteArgument(g_hVote, "Config kapatilsin mi?");
		SetBuiltinVoteInitiator(g_hVote, client);
...
public ResetMatchVoteResultHandler(Handle:vote, num_votes, num_clients, const client_info[][2], num_items, const item_info[][2])
{
	for (new i=0; i<num_items; i++)
	{
		if (item_info[i][BUILTINVOTEINFO_ITEM_INDEX] == BUILTINVOTES_VOTE_YES)
		{
			if (item_info[i][BUILTINVOTEINFO_ITEM_VOTES] > (num_votes / 2))
			{
				DisplayBuiltinVotePass(vote, "Config kapatiliyor...");
