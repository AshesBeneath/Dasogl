...
public OnPluginStart()
{
	...
	l4d_ready_live_sound = CreateConVar("l4d_ready_live_sound", "ui/pickup_guitarriff10.wav", "The sound that plays when a round goes live");
...
public OnMapStart()
{
	...
	PrecacheSound("ui/pickup_guitarriff10.wav");
...
public Action:KickSpecs_Cmd(client, args)
{
	if (IsClientInGame(client) && L4D2Team:GetClientTeam(client) != L4D2Team_Spectator)
	{
		if (IsNewBuiltinVoteAllowed())
		{
			...
			Format(sBuffer, 64, "Admin veya Caster Olmayan Tum Izleyiciler Atilsin mi?");
...
UpdatePanel()
{
	...
	if (bufLen != 0)
	{
		...
		DrawPanelText(menuPanel, "->1. Sag Kalanlar");
	...
	if (bufLen != 0)
	{
		...
		DrawPanelText(menuPanel, "->2. Enfekteler");
	...
	if (bufLen)
	{
		...
		DrawPanelText(menuPanel, "->3. Caster");
	...
	if (bufLen != 0)
	{
		...
		if (casterCount > 0)
		{
			DrawPanelText(menuPanel, "->4. Izleyiciler");
		}
		else
		{
			DrawPanelText(menuPanel, "->3. Izleyiciler");
		}
...
public Action:ReadyCountdownDelay_Timer(Handle:timer)
{
	if (readyDelay)
	{
		PrintHintTextToAll("%d", readyDelay);
	...
	if (GetConVarBool(l4d_ready_enable_sound))
	{
		...
		{
			EmitSoundToAll("ui/pickup_guitarriff10.wav", -2, 0, 75, 0, 0.5);
		}
	}
	return Plugin_Stop;
}
...
CancelFullReady()
{
	if (readyCountdownTimer != INVALID_HANDLE)
	{
		...
		readyCountdownTimer = INVALID_HANDLE;
		PrintHintTextToAll("Bir muhterem iptal etti");
	}
}
