...
FreezePoints( bool:show_message = false ) {
    if ( !bTankAlive ) {
        ...
        if ( show_message ) CPrintToChatAll("{red}[{default}No Tank Rush{red}] {olive}Puanlar donduruldu. {default}O tank ölcek len !!!");
...
UnFreezePoints( bool:show_message = false ) {
    if ( bTankAlive ) {
        if ( show_message ) CPrintToChatAll("{red}[{default}No Tank Rush{red}] {olive}Puanlar tekrar işlenmektedir.");
